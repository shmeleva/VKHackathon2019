...
export const checkUserGoal = async (userId: string, goalId: string, date: Date): Promise<void> => {
    await GoalModel.findOneAndUpdate({ userId, _id: goalId }, { "$push": { "checks": date } });
};

export const uncheckUserGoal = async (userId: string, goalId: string, date: Date): Promise<void> => {
    await GoalModel.findOneAndUpdate({ userId, _id: goalId }, { "$pull": { "checks": date } });
};

export const donateToUserGoal = async (goalId: string, amount: number): Promise<void> => {
